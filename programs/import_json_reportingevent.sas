...
filename in "&sasbaseard.\data\jsonmd\Common Safety Displays.json" encoding="utf-8";
/**OR**/
...
filename map "&sasbaseard.\programs\reportingevent_md.map";

libname in json map=map automap=reuse;

/* Set the library for the metadata read from JSON */
...
proc sql noprint;
	create table jsonmd.Categorizations (drop = tabord ordinal_subcategorizations) as
...
%macro get_nlist(list);	
	...
	proc transpose data = _tmp_&list
                   out = %sysfunc(propcase(jsonmd.&list.))
...
proc sql noprint;
	create table jsonmd.Outputs (drop = ordinal_outputs) as
...
proc sql noprint;
	create table jsonmd.Displays as
...
%macro get_wc(wctype /* Where clause type: should be dataSubsets or analysisSets */);
	...
	data jsonmd.&wctype (rename = (logicalOperator = compndExpression_logicalOperator
...
%macro get_results;
	...
	proc sql noprint;
		...
		create table jsonmd.AnalysisResults as
		select a.id,
...
/***********************************************************************************\
* Several datasets contain level and order as character variables whereas it would  *
* make more sense for them to be numeric.
*                                                                                   *
* So we now search for the affected datasets and convert the relevant variables to  *
* numeric.
\***********************************************************************************/

/* The CONVERT_TO_NUMERIC macro searches the specified library for any datasets
   containing level and/or order variables as character type variables.
   For each one, it then converts those variables to numeric. */

%macro convert_to_numeric (
	libin /* library to search */ );

	proc datasets;
		contents data = &libin.._all_ out = work._tmp_conts;
	run;
	quit;

	* Get list of affected datasets;
	%local dslist varlist tmpvarlist renlist;
	proc sql;
		select distinct memname into :dslist separated by ' '
			from _tmp_conts
			where name in ('order', 'level', 'group_level') and type = 2;
	quit;

	* Loop through datasets;
	%local ids ds ivar varname;
	%let ids = 1;
	%do %while(%scan(&dslist., &ids., ' ') ne );
		%let ds = %scan(&dslist., &ids., ' ');
		%put NOTE: Dataset &ds.;

		* Get list of affected variables;
		proc sql;
			select name into :varlist separated by ' '
				from _tmp_conts
				where name in ('order', 'level', 'group_level') and type = 2 and memname = "&ds.";
		quit;
		%put NOTE: Variables &varlist.;

		* Build strings to use in the conversion by looping through the variables;
		%let tmpvarlist = %str();
		%let renlist = %str();
		%let ivar = 1;
		%do %while(%scan(&varlist., &ivar., ' ') ne );
			%let varname = %scan(&varlist., &ivar., ' ');
			%let tmpvarlist = &tmpvarlist. tmp_&varname.;
			%let renlist = &renlist. tmp_&varname. = &varname.;
			%let ivar = %eval(&ivar+1);
		%end;

		* Convert relevant variables in this dataset from character to numeric;
		data &libin..&ds. ( drop = &varlist. );
			set &libin..&ds.;
			length &tmpvarlist. 8;
			%let ivar = 1;
			%do %while(%scan(&varlist., &ivar., ' ') ne );
				%let varname = %scan(&varlist., &ivar., ' ');
				tmp_&varname. = input(&varname., 8.);
				%let ivar = %eval(&ivar+1);
			%end;
		data &libin..&ds.;
			set &libin..&ds. ( rename = ( &renlist. ) );
		run;
